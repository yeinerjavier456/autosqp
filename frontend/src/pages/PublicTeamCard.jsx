...
const PublicTeamCard = () => {
  ...
  const primaryColor = company.primary_color || '#facc15';
  const secondaryColor = company.secondary_color || '#081225';
  const mapsUrl = getMapsUrl(company.contact_address);
  ...
  return (
    <main
      ...
      style={{
        background: `linear-gradient(145deg, ${secondaryColor} 0%, ${withAlpha(secondaryColor, 'f2')} 54%, ${withAlpha(primaryColor, 'd6')} 100%)`,
      }}
    >
      <section className="mx-auto w-full max-w-[390px] overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="relative min-h-[260px] overflow-hidden px-7 py-7 text-white" style={{ background: secondaryColor }}>
          <div className="absolute -right-16 bottom-0 h-48 w-72 rounded-tl-full opacity-50" style={{ background: `radial-gradient(circle at 30% 50%, ${withAlpha(primaryColor, '88')}, transparent 62%)` }} />
          ...
          <div className="relative z-10 flex items-center gap-3">
            <PublicBrandLogo
              ...
              textClassName="text-base font-black uppercase tracking-wide text-white"
          ...
          <div className="relative z-10 mt-8">
            <p className="text-3xl font-black leading-tight">
              Gracias por confiar <span style={{ color: primaryColor }}>en nosotros</span>
            </p>
            <p className="mt-4 max-w-[230px] text-sm leading-6 text-white/85">
              Estamos aquí para ayudarte a encontrar el carro ideal.
            </p>
          </div>
          <div className="absolute bottom-6 left-7 grid grid-cols-4 gap-1">
            {Array.from({ length: 12 }).map((_, index) => (
              <span key={index} className="h-1 w-1 rounded-full" style={{ background: primaryColor }} />
            ))}
          </div>
        </div>

        <div className="relative -mt-9 mx-3 rounded-[28px] bg-white px-6 pb-6 pt-5 shadow-xl">
          <span className="absolute -top-8 right-5 flex h-14 w-14 items-center justify-center rounded-full border-4 border-white text-xs font-black text-white" style={{ background: secondaryColor }}>
            GIF
          </span>
          <div className="grid grid-cols-[112px_1fr] gap-5">
            ...
            <div className="min-w-0 pt-5">
              <h1 className="break-words text-2xl font-black leading-tight text-slate-950">{card.full_name}</h1>
              <p className="mt-1 text-sm font-black uppercase tracking-wide text-slate-700">{card.position || 'Equipo comercial'}</p>
              <a href={`mailto:${card.email}`} className="mt-4 flex min-w-0 items-center gap-2 text-sm font-semibold text-slate-700">
                <span className="shrink-0">✉</span>
                <span className="truncate">{card.email}</span>
              </a>
              <div className="mt-6 h-0.5 w-full" style={{ background: primaryColor }} />
            </div>
          </div>

          <div className="mt-7 rounded-2xl bg-slate-50 p-5">
            <div className="flex gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-white" style={{ background: secondaryColor }}>
              ...
              <div>
                <p className="text-lg font-black text-slate-950">Visítanos en nuestra empresa</p>
            ...
            {company.contact_address ? (
              <p className="mt-4 text-center text-sm font-semibold text-blue-600">{company.contact_address}</p>
            ) : null}
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2 px-5 py-7 text-center text-xs font-semibold text-white" style={{ background: secondaryColor }}>
