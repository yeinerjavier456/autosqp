...
const VehicleForm = () => {
    ...
    const fetchModels = async (brandId) => {
        try {
            const response = await axios.get(`http://localhost:8001/brands/${brandId}/models/`);
    ...
    const fetchVehicle = async () => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:8001/vehicles/${id}`, {
    ...
    const handleSeedBrands = async () => {
        try {
            setLoading(true);
            const response = await axios.post('http://localhost:8001/seed/brands');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (id) {
                // Update
                await axios.put(`http://localhost:8001/vehicles/${id}`, payload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                Swal.fire('Éxito', "Vehículo actualizado correctamente", 'success');
            } else {
                // Create
                await axios.post('http://localhost:8001/vehicles/', payload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
