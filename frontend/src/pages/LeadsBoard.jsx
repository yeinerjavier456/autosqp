...
const LeadsBoard = () => {
    ...
    const fetchAvailableVehicles = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get('http://localhost:8001/vehicles/?status=available', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const confirmStatusChange = async () => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:8001/leads/${leadId}`,
                {
                    status: newStatus,
    ...
    const handleConfirmSale = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:8001/sales/', payload, {
                headers: { Authorization: `Bearer ${token}` }
            });

            await axios.put(`http://localhost:8001/leads/${selectedLeadForSale.id}`,
