...
const CreditBoard = () => {
    ...
    const fetchCredits = async () => {
        try {
            ...
            const response = await axios.get('http://3.234.117.124:8000/credits', {
    ...
    const handleDragEnd = async (result) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://3.234.117.124:8000/credits/${creditId}`,
                { status: newStatus },
                { headers: { Authorization: `Bearer ${token}` } }
            );
        } catch (error) {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://3.234.117.124:8000/credits', payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
