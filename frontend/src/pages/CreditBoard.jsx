...
const CreditBoard = () => {
    ...
    const fetchCredits = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:8001/credits', {
    ...
    const handleDragEnd = async (result) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:8001/credits/${creditId}`,
                { status: newStatus },
                { headers: { Authorization: `Bearer ${token}` } }
            );
        } catch (error) {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8001/credits', payload, {
                headers: { Authorization: `Bearer ${token}` }
            });
