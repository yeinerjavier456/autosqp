...
const InventoryList = () => {
    ...
    const fetchVehicles = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://54.226.30.192:8000/vehicles/', {
                params,
    ...
    const handleMarkSold = async (vehicleId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://54.226.30.192:8000/vehicles/${vehicleId}`,
                { status: 'sold' },
                { headers: { Authorization: `Bearer ${token}` } }
            );
                ...
                <div className="mt-4 md:mt-0 flex gap-2">
                    <button
                        onClick={async () => {
                            ...
                            try {
                                const token = localStorage.getItem('token');
                                await axios.post('http://54.226.30.192:8000/vehicles/', demoVehicle, {
                                    headers: { Authorization: `Bearer ${token}` }
                                });
                                fetchVehicles();
